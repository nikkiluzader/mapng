...
export const fetchTerrainData = async (
    ...
    if (includeOSM && osmFeatures.length > 0) {
        const defaultVisibility = { 'Roads': true };
        onProgress?.("Generating OSM texture...");
        terrainData.osmTextureUrl = await generateOSMTexture(terrainData, defaultVisibility);
        onProgress?.("Generating Hybrid texture...");
        terrainData.hybridTextureUrl = await generateHybridTexture(terrainData, defaultVisibility);
    }
...
export const addOSMToTerrain = async (
    ...
    if (osmFeatures.length > 0) {
        const defaultVisibility = { 'Roads': true };
        onProgress?.("Generating OSM texture...");
        newTerrainData.osmTextureUrl = await generateOSMTexture(newTerrainData, defaultVisibility);
        onProgress?.("Generating Hybrid texture...");
        newTerrainData.hybridTextureUrl = await generateHybridTexture(newTerrainData, defaultVisibility);
    }
